...
def train_and_test(dataset, nb_epochs, degree, random_seed, label,
    ...
    trainx, trainy,trainDf = data.get_train(label)
    Deposit = np.reshape(trainDf, (79027, 1))
    if enable_early_stop:
        validx, validy = data.get_valid(label)
    # validx, validy = data.get_valid(label)
    trainx_copy = trainx.copy()
    testx, testy,testDf= data.get_test(label)
    ...
    with tf.name_scope('loss_functions'):
        ...
        # Deposit constraints loss
        # W = 300
        # Cross_entroy_1 = -Deposit_pl*tf.log(tf.clip_by_value(a,1e-10,1.0))*W
        # Cross_entroy_1 = -Deposit_pl * tf.log(tf.clip_by_value(a, 1e-10, 1.0))
        # Cross_entroy_0 = -tf.subtract(tf.ones_like(a),Deposit_pl)*tf.log(tf.clip_by_value(tf.subtract(tf.ones_like(a), a),1e-10,1.0))
        # Constraint_loss = tf.add(Cross_entroy_0, Cross_entroy_1)
        # cost_Constraint = tf.reduce_mean(Constraint_loss)
        cost_Constraint=tf.reduce_mean(tf.square(a-Deposit_pl))
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
        ...
        train_and_test('arrhythmia', 300, 1, 100, 1,
