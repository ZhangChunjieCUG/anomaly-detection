...
def train_and_test(dataset, nb_epochs, degree, random_seed, label,
    ...
    data = importlib.import_module("data.arrhythmia")
    ...
    z_pl = tf.placeholder(tf.float32, shape=[None, latent_dim],
                          name="input_z")
    Deposit_pl = tf.placeholder(tf.float32,
                                name="input_Deposit")
    temp1 = tf.placeholder(tf.float32,
                                name="input_temp")
    ...
    trainx, trainy, trainDf, traintemp = data.get_train(label)
    Deposit = np.reshape(trainDf, (94080, 1))
    train_temp=np.reshape(traintemp,(94080,1))

    # if enable_early_stop:
    #     validx, validy = data.get_valid(label)
    # validx, validy = data.get_valid(label)
    trainx_copy = trainx.copy()
    testx, testy, testDf,testtemp = data.get_test(label)
    validx, validy,valiaDf, vaildtemp = data.get_valid(label)
    ...
    with tf.variable_scope('encoder_model'):
        z_gen = enc(x_pl, is_training=is_training_pl,
                    do_spectral_norm=do_spectral_norm)  # 对X输入样本进行编码E
    ...
    with tf.name_scope('loss_functions'):
        ...
        # Constaint deposit (Zhang)
        R_error = rec_x - x_pl
        R_error = tf.contrib.layers.flatten(R_error)
        score_error = tf.norm(R_error, ord=2, axis=1,
                              keep_dims=False, name='d_loss')
        b = tf.reshape(score_error, [-1, 1])
        Deposit_pl = tf.reshape(Deposit_pl, [-1, 1])
        train_temp1=tf.reshape(temp1,[-1,1])
        n = 1
        a = tf.tanh(b)
        lossji=tf.square(a-Deposit_pl)
        loaa=tf.multiply(lossji,train_temp1)
        cost_Constraint=tf.reduce_mean(loaa)
        ...
        loss_generator = gen_loss_xz + cycle_consistency_loss + n * cost_Constraint
        loss_encoder = enc_loss_xz + cycle_consistency_loss + n * cost_Constraint
    ...
    with tf.name_scope('Testing'):
        ...
        with tf.variable_scope('Scores'):
            ...
            rec_error_valid = tf.reduce_mean(score_l1)
    ...
    with sv.managed_session(config=config) as sess:
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            train_loss_dis, train_loss_gen, train_loss_enc, Constraint_loss_2 = [0, 0, 0, 0, 0, 0, 0]
            ...
            for t in range(nr_batches_train):
                ...
                feed_dict = {x_pl: trainx[ran_from:ran_to],
                             Deposit_pl: Deposit[ran_from:ran_to],
                             temp1:train_temp[ran_from:ran_to],
                             z_pl: np.random.normal(size=[batch_size, latent_dim]),
                             is_training_pl: True,
                             learning_rate: lr}

                _, _, _, ld, ldxz, ldxx, ldzz, step,R_error1 = sess.run([train_dis_op_xz,
                                                                train_dis_op_xx,
                                                                train_dis_op_zz,
                                                                loss_discriminator,
                                                                dis_loss_xz,
                ...
                feed_dict = {x_pl: trainx_copy[ran_from:ran_to],
                             ...
                             is_training_pl: True,
                             learning_rate: lr}
                X_p_, Deposit_pl_, Z_PL_, R_error1, score_error1, a1, z_fake_gen_, z_logot_fake_1 = sess.run(
                    [x_pl, Deposit_pl, z_pl, R_error, score_error, a, z_fake_gen, z_logit_fake],
                    feed_dict=feed_dict)

                _, _, le, lg, rec_x_, Constraint_loss_, score_error_, z_gen_ = sess.run([train_gen_op,
                                                                                         train_enc_op,
                                                                                         loss_encoder,
                                                                                         loss_generator,
                                                                                         rec_x,
                ...
                train_loss_gen += lg
                train_loss_enc += le
                Constraint_loss_2 += Constraint_loss_
            ...
            Constraint_loss_2 /= nr_batches_train

            Con_loss.append(Constraint_loss_2)
            train_loss_gen1.append(train_loss_gen)
            train_loss_enc1.append(train_loss_enc)
            dis_loss_dis.append(train_loss_dis)
            epoch1.append(epoch)
            ...
            if allow_zz:
                print("Epoch %d | time = %ds | loss gen = %.4f | loss enc = %.4f | "
                      ...
                      "loss dis zz = %.4f| loss Constraint = %.4f"
                      % (epoch, time.time() - begin, train_loss_gen,
                         train_loss_enc, train_loss_dis, train_loss_dis_xz,
                         train_loss_dis_xx, train_loss_dis_zz, Constraint_loss_2))
            ...
            epoch += 1
        lossvalue = np.vstack([epoch1, train_loss_enc1, train_loss_enc1, dis_loss_dis, Con_loss])
        np.savetxt('lossconstraint.csv', lossvalue, delimiter=',')
        ...
        vl, lat = sess.run([rec_error_valid, rec_z], feed_dict=feed_dict)
        valid_loss += vl
        scores_Vaildl1+= sess.run(score_l1, feed_dict=feed_dict).tolist()
        model = 'alad_sn{}_dzz{}'.format(do_spectral_norm, allow_zz)
        save_results(scores_Vaildl1, validy, model, dataset, 'l1',
        ...
        # np.savetxt('Constraint_loss_result.csv', Constraint_loss_result, delimiter=',')
        ...
        scores_l1_1 = np.reshape(scores_l1, (336, 336), order='F')

        scores_l2_1 = np.reshape(scores_l2, (336, 336), order='F')

        np.savetxt('scores_l1.csv', scores_l1, delimiter=',')
        np.savetxt('scores_l2.csv', scores_l2, delimiter=',')

        np.savetxt('scores_scores_l1.csv', scores_l1_1, delimiter=',')
        np.savetxt('scores_scores_l2.csv', scores_l2_1, delimiter=',')
        # np.savetxt('scores_scores_fm.csv', scores_fm_1, delimiter=',')
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
        ...
        train_and_test('arrhythmia', 100, 1, 100, 1,
